...
model = MGLVAE(channels=nchannels, dim_z=dim_z, dim_beta=dim_beta, K=K, arch=args.arch, device=device).to(device)
...
def train_epoch(model, epoch, train_loader, optimizer, cuda=False, log_interval=10):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        recon_batch, mu_z, var_z, pk, mu_beta, var_beta, mus_beta, vars_beta = model.forward(data)
        loss, rec, klz, klbeta, klalpha = model.loss_function(recon_batch, data, mu_z, var_z, pk, mu_beta, var_beta, mus_beta, vars_beta, distribution)
        loss.backward()
    ...
    return train_loss / nims, train_rec / nims, \
           train_klz / nims, train_klbeta / nims, train_klalpha / nims
...
def test(model, epoch, test_loader, cuda=False, model_name='model'):
    ...
    with torch.no_grad():
        ...
        for i, (data, _) in enumerate(test_loader):
            data = data.to(device).view(-1, nchannels, data.shape[-2], data.shape[-1])
            recon_batch, mu_z, var_z, pk, mu_beta, var_beta, mus_beta, vars_beta = model.forward(data)
            loss, rec, klz, klbeta, klalpha = model.loss_function(recon_batch, data, mu_z, var_z, pk, mu_beta, var_beta,
                                                                  mus_beta, vars_beta, distribution)
            test_loss += loss.item()
            test_rec += rec.item()
            test_klz += klz.item()
    ...
    return test_loss, test_rec, test_klz, test_klbeta, test_klalpha
...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        tr_loss, tr_rec, tr_klz, tr_klbeta, tr_klalpha = train_epoch(model, epoch, train_loader, optimizer,
        ...
        test_loss, test_rec, test_klz, test_klbeta, test_klalpha = test(model, epoch, test_loader,
