...
def plot_global_latent(nsamples, nreps, nims):
    model.eval()
...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.epochs + 1):
        tr_loss, tr_rec, tr_kl_l, tr_kl_g = train(epoch)
        tr_losses.append(tr_loss)
        tr_recs.append(tr_rec)
        tr_kl_ls.append(tr_kl_l)
        tr_kl_gs.append(tr_kl_g)
        test_loss, test_rec, test_kl_l, test_kl_g = test(epoch)
        ...
        with torch.no_grad():
            ...
            if np.mod(epoch, 100)==0 or epoch==1 or epoch==args.epochs:
                save_model(epoch)
                plot_latent(epoch)
                plot_global_latent(nsamples=4, nreps=1000, nims=20 )
