...
class Interpolation():
    ...
    def build(self, model, batch_1, batch_2, folder='./'):
        ...
        mu_z1, var_z1, p1, mu_beta1, var_beta1 = model._encode(batch_1)
        mu_z2, var_z2, p2, mu_beta2, var_beta2 = model._encode(batch_2)
        ...
        # Interpolation in K
        mus_beta, vars_beta = model._beta_mix(torch.eye(model.K).to('cpu'))
        grid = []
        for s1 in range(self.steps):
...
if __name__ == "__main__":
    ...
    model = MGLVAE(channels=nchannels[args.dataset], dim_z=dim_z, dim_beta=dim_beta, K=K, arch=args.arch)
