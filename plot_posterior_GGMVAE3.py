...
parser.add_argument('--global_points', type=int, default=400, metavar='N',
                    help='Number of local points to plot')
...
parser.add_argument('--model_name', type=str, default='GGMVAE3/celeba_faces',
...
model = GGMVAE3(channels=nchannels[args.dataset], dim_z=args.dim_z, dim_beta=args.dim_beta, dim_w=args.dim_w, K=args.K, arch=args.arch, device=device).to(device)
...
for i in range(args.global_points):
    ...
    recon_batch, mu_z, var_z, mu_beta, var_beta, mus_z_p, vars_z_p, mu_w, var_w, pi = model(batch)
...
elif args.dataset =='celeba_faces':
    ########################################################################################################################
    fig, ax = plt.subplots(figsize=(6, 6))
    if args.attribute != '':
        dim = data_tr.dataset.get_label_index(args.attribute)
        ...
        [plt.scatter(X_l[labels_l==l, 0], X_l[labels_l==l, 1], alpha=0.5) for l in range(2)]
    plt.title('Local space')
