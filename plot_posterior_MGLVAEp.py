...
parser.add_argument('--epoch', type=int, default=100,
                    help='Epoch to load')
parser.add_argument('--local_points', type=int, default=200, metavar='N',
...
parser.add_argument('--model_name', type=str, default='mix_p/mnist',
...
model = MGLVAEp(channels=nchannels[args.dataset], dim_z=args.dim_z, dim_beta=args.dim_beta, K=args.K, arch=args.arch)
...
for i in range(args.global_points):
    ...
    recon, mu, var, pk, mu_g_, var_g_ = model(batch)

    mu_l.append(mu)
    var_l.append(var)

    mu_g.append(mu_g_[torch.argmax(pk)])
    var_g.append(var_g_[torch.argmax(pk)])
...
if args.dataset == 'celeba' or args.dataset == 'light_celeba':
    ...
    for i in range(args.global_points):
        ...
        recon, mu, var, pk, mu_g_, var_g_ = model.forward(batch)
    ...
    for s in range(len(series)):
        ...
        for x, std in zip(X, stds):
            ...
            ax.add_artist(el)
    """
    [plt.scatter(X_g_series[idxs[s], 0], X_g_series[idxs[s], 1], color=colors[s], alpha=0.5, label=series[s]['name']) for s in
     range(len(series))]
    plt.gca().set_prop_cycle(None)
    """
    idxs = [np.where(labels_g_off == s) for s in range(len(series))]
    [plt.scatter(X_g_off[idxs[s], 0], X_g_off[idxs[s], 1], marker='*', color=colors[s], alpha=0.5, zorder=8) for s in
     range(len(series))]
    #plt.scatter(mus_beta.detach().numpy()[:, 0], mus_beta.detach().numpy()[:, 1], c='k', label=r'$p(\beta)$')

    mus_beta, vars_beta =  model._beta_mix(torch.eye(model.K).to(model.device))
    ...
    """
    plt.legend(loc='best')
    plt.savefig(folder + 'global_space_' + str(args.epoch) + '_ops' + '_' + args.dim_reduction + '.png')
    ########################################################################################################################
