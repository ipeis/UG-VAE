...
parser.add_argument('--model_name', type=str, default='GGMVAE2/celeba_tanh',
...
model = GGMVAE2(channels=nchannels, dim_z=dim_z, dim_beta=dim_beta, dim_w=dim_w, K=K, arch=args.arch, device=device).to(device)
...
def train_epoch(model, epoch, train_loader, optimizer, cuda=False, log_interval=10):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        recon_batch, mu_z, var_z, mu_beta, var_beta, mu_w, var_w, pi = model(data)
        loss, rec, klz, klalpha, klw = model.loss_function(recon_batch, data, mu_z, var_z, mu_beta, var_beta, mu_w, var_w, pi, distribution)
        loss.backward()
...
def test(model, epoch, test_loader, cuda=False, model_name='model'):
    ...
    with torch.no_grad():
        ...
        for i, (data, _) in enumerate(test_loader):
            data = data.to(device).view(-1, nchannels, data.shape[-2], data.shape[-1])
            recon_batch, mu_z, var_z, mu_beta, var_beta, mu_w, var_w, pi = model(data)
            loss, rec, klz, klalpha, klw = model.loss_function(recon_batch, data, mu_z, var_z, mu_beta, var_beta,
