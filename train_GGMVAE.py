...
parser.add_argument('--dim_z', type=int, default=50, metavar='N',
...
parser.add_argument('--model_name', type=str, default='GGMVAE/celeba_faces_50_noKL',
...
model = GGMVAE(channels=nchannels, dim_z=dim_z, dim_beta=dim_beta, dim_w=dim_w, K=K, arch=args.arch, device=device).to(device)
...
def train_epoch(model, epoch, train_loader, optimizer, cuda=False, log_interval=10):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        recon_batch, mu_z, var_z, mu_beta, var_beta, mu_w, var_w, pi, mus_beta_p, vars_beta_p = model(data)
        loss, rec, klz, klbeta, klalpha, klw = model.loss_function(recon_batch, data, mu_z, var_z, mu_beta, var_beta, mu_w, var_w, pi, mus_beta_p, vars_beta_p, distribution)
        loss.backward()
    ...
    return train_loss / nims, train_rec / nims, \
           train_klz / nims, train_klbeta / nims, train_klalpha / nims, train_klw / nims
...
def test(model, epoch, test_loader, cuda=False, model_name='model'):
    ...
    with torch.no_grad():
        ...
        for i, (data, _) in enumerate(test_loader):
            data = data.to(device).view(-1, nchannels, data.shape[-2], data.shape[-1])
            recon_batch, mu_z, var_z, mu_beta, var_beta, mu_w, var_w, pi, mus_beta_p, vars_beta_p = model(data)
            loss, rec, klz, klbeta, klalpha, klw = model.loss_function(recon_batch, data, mu_z, var_z, mu_beta, var_beta,
                                                                       mu_w, var_w, pi, mus_beta_p, vars_beta_p,
                                                                       distribution)
            test_loss += loss.item()
            test_rec += rec.item()
            test_klz += klz.item()
    ...
    return test_loss, test_rec, test_klz, test_klbeta, test_klalpha, test_klw
...
def plot_losses(model, tr_losses, test_losses, tr_recs, test_recs,
    ...
    plt.semilogy(tr_klbetas, color=colors[3], label=r'$KL_\beta$')
    plt.semilogy(test_klbetas, color=colors[3], linestyle=':')
...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        tr_loss, tr_rec, tr_klz, tr_klbeta, tr_klalpha, tr_klw = train_epoch(model, epoch, train_loader, optimizer,
        ...
        test_loss, test_rec, test_klz, test_klbeta, test_klalpha, test_klw = test(model, epoch, test_loader,
        ...
        with torch.no_grad():
            ...
            plot_losses(model, tr_losses, test_losses, tr_recs, test_recs,
                        tr_klzs, test_klzs, tr_klbetas, test_klbetas, tr_klalphas, test_klalphas,
                        tr_klws, test_klws,
